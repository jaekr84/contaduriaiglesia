...
export function ExchangeForm() {
    ...
    return (
        <form onSubmit={handleSubmit} className="w-full">
            <fieldset disabled={isPending} className="flex flex-col lg:flex-row gap-4 items-end disabled:opacity-70">
                ...
                <div className="w-full lg:w-32 space-y-1.5">
                    ...
                    <input
                        ...
                        className="w-full h-10 rounded-lg border-zinc-200 bg-zinc-50/50 px-3 text-sm transition-all focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 dark:border-zinc-800 dark:bg-zinc-900 dark:focus:bg-zinc-950"
                    />
                </div>

                {/* Operation Toggle */}
                <div className="w-full lg:w-auto flex-shrink-0 space-y-1.5">
                    <label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Operación</label>
                    <div className="flex bg-zinc-100 p-1 rounded-lg dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 h-10 w-full lg:w-auto">
                        <button
                            type="button"
                            onClick={() => setOperation('SELL_USD')}
                            className={cn(
                                "flex-1 lg:flex-none px-4 py-1 rounded-md text-sm font-medium transition-all flex items-center justify-center gap-2",
                                operation === 'SELL_USD'
                ...
                {/* Amount Out (Entregas) */}
                <div className="w-full lg:w-48 space-y-1.5 relative group">
                    <label className={cn(
                        "text-xs font-semibold uppercase tracking-wider flex justify-between",
                    ...
                    <div className="relative">
                        <MoneyInput
                            ...
                            placeholder="0,00"
                            className={cn(
                                "w-full h-10 pl-3 pr-3 text-right font-medium rounded-lg border transition-all focus:ring-2",
                                operation === 'SELL_USD'
                        ...
                        />
                        <div className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold opacity-50 pointer-events-none">
                            {currencyOut === 'USD' ? '$' : '$'}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 min-w-[80px] flex flex-col items-center justify-center pb-2 gap-1 px-2">
                    {rateDisplay && (
                        <div className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-600 border border-zinc-200 whitespace-nowrap dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-400">
                            {rateDisplay}
                        </div>
                    )}
                    <div className="h-[2px] w-full bg-gradient-to-r from-transparent via-zinc-200 to-transparent dark:via-zinc-800 relative flex items-center justify-center">
                        <div className="w-6 h-6 rounded-full bg-white border border-zinc-200 shadow-sm flex items-center justify-center text-zinc-400 dark:bg-zinc-900 dark:border-zinc-800">
                ...
                <button
                    type="submit"
                    disabled={isPending}
                    className="w-full lg:w-auto h-10 inline-flex items-center justify-center rounded-lg bg-zinc-900 px-6 text-sm font-medium text-zinc-50 shadow-sm hover:translate-y-[-1px] hover:shadow-md transition-all active:translate-y-[1px] disabled:pointer-events-none disabled:opacity-50 dark:bg-zinc-50 dark:text-zinc-900"
                >
                    {isPending ? (
